...
export class CreatePollComponentComponent implements OnInit {
  ...
  submitted = false;
  dayGridMonthSelectedBefore = false;
  ...
  ngOnInit(): void {
    ...
    this.options = {
      ...
      select: (selectionInfo) => {
        if (this.dayGridMonthSelectedBefore) {
          this.dayGridMonthSelectedBefore = false;
        } else {
          if (this.calendarComponent.getApi().view.type == 'dayGridMonth' && selectionInfo.end.getTime() === selectionInfo.start.getTime() + 24 * 60 * 60 * 1000) {
            this.dayGridMonthSelectedBefore = true;
            this.calendarComponent
              .getApi()
              .changeView('timeGridWeek', selectionInfo.start);
            this.calendarComponent.getApi().select(selectionInfo.start);
          } else {
            console.log(selectionInfo);
            const calendarApi = this.calendarComponent.getApi();
            console.log(this.getUniqueId(8));

            const hourOfBeginning = selectionInfo.start.getHours();
            if (hourOfBeginning >= 20 || hourOfBeginning < 8) {
              selectionInfo.start.setHours(8);
            }
            const hourOfEnding = selectionInfo.end.getHours();
            if (hourOfEnding >= 20 || hourOfEnding < 8) {
              selectionInfo.end.setDate(selectionInfo.end.getDate() - 1);
              selectionInfo.end.setHours(20);
            }
            const evt = {
              title: '',
  ...
  nextPage1(): void {
    ...
    if (this.poll.id == null) {
      ...
      this.events
        .filter(
          (c) => c.extendedProps != null && c.extendedProps.choiceid != null
        )
        .forEach((e) => {
          toKeep.push(
            this.poll.pollChoices.filter(
              (c1) => c1.id === e.extendedProps.choiceid
            )[0]
          );
        });
      ...
      this.poll.pollChoices.forEach((c) => {
        const res = this.events.filter(
          (c1) =>
            c1.extendedProps != null &&
            c1.extendedProps.choiceid != null &&
            c1.extendedProps.choiceid === c.id
        )[0];
  ...
  getICS(): void {
    this.loadics = true;
    this.pollService.getICS(this.slugid, this.ics).subscribe(
      (res) => {
        this.loadics = false;
